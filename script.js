...
async function renderPublishedResult(dom, application) {
  ...
  try {
    await renderQrImage(dom.resultQrImage, fullUrl);
  } catch {
...
function normaliseEmbeddedApplication(input) {
  ...
  const application = {
    ...
    hours: toCleanString(input.hours) || toCleanString(input.h),
    workplaceType: toCleanString(input.workplaceType) || toCleanString(input.wp),
    ...
    companySummary: toCleanString(input.companySummary) || toCleanString(input.cs),
    roleSummary: toCleanString(input.roleSummary) || toCleanString(input.rs),
    headlineAttraction: toCleanString(input.headlineAttraction) || toCleanString(input.ha),
    rolePurpose: toCleanString(input.rolePurpose) || toCleanString(input.rp),
    travelRequired: toCleanString(input.travelRequired),
    ...
    companyPridePoints: normaliseStringArray(input.companyPridePoints || input.cpp),
    coreResponsibilities: normaliseStringArray(input.coreResponsibilities),
    essentialRequirements: normaliseStringArray(input.essentialRequirements),
    preferredRequirements: normaliseStringArray(input.preferredRequirements),
    skillsWanted: normaliseStringArray(input.skillsWanted),
